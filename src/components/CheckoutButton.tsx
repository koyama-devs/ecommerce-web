...
export type StoreInfo = {
  ...
  email: string;
  taxId?: string;
  logoUrl?: string; // tạm thời dùng URL, bạn sẽ thay bằng logo thật
};
...
export type InvoiceData = {
  ...
  invoice: {
    ...
    date: string; // ISO or locale
    orderId: string;
    paymentMethod: string; // card, bank, COD...
  ...
  totals: {
    ...
    vatRate?: number; // ví dụ 0.1 = 10%
  };
  extras?: {
    terms?: string;
    thanksNote?: string;
    signer?: string; // chữ ký/đại diện
  };
};
...
/** Helper: load ảnh từ URL -> dataURL, để nhúng vào jsPDF */
async function loadImageAsDataURL(url?: string): Promise<string | null> {
  ...
  try {
    ...
    return null; // nếu CORS chặn, vẫn tiếp tục không logo
  }
}

/** ====== HÀM DÙNG CHUNG: Tạo PDF hóa đơn (đồng bộ cho web & sau thanh toán) ====== */
export async function generateInvoicePDF(data: InvoiceData) {
  ...
  doc.text("HÓA ĐƠN THANH TOÁN", pageWidth - marginX, y + 20, {
    align: "right",
  });
  ...
  storeLines.forEach((line, idx) => {
    doc.text(line, marginX, y + 100 + idx * 14);
  });

  y += 130;

  // Invoice Meta & Customer Info (2 cột)
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  ...
  const rightMeta = [
    ...
    data.customer.shippingAddress
      ? `Đ/c giao hàng: ${data.customer.shippingAddress}`
      : "",
  ...
  doc.text(formatJPY(data.totals.grandTotal), summaryX + 180, yy + 8, {
    align: "right",
  });

  // Điều khoản / lời cảm ơn / chữ ký
  const blockY = yy + 40;
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  const terms =
